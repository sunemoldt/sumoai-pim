import { createClient } from "https://esm.sh/@supabase/supabase-js@2.49.1";

const corsHeaders = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type",
};

const SUPABASE_URL = Deno.env.get("SUPABASE_URL")!;
const SUPABASE_ANON_KEY = Deno.env.get("SUPABASE_ANON_KEY")!;
const SUPABASE_SERVICE_ROLE_KEY = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
const API_VERSION = "2026-04";
const VAT_RATE = 0.25;

// HTML -> Shopify rich_text_field JSON (root/heading/paragraph/list/list-item/text/link).
function htmlToShopifyRichText(html: string): string {
  const clean = String(html ?? "").trim();
  if (!clean) return JSON.stringify({ type: "root", children: [] });
  type Node = { type: string; children?: Node[]; value?: string; level?: number; url?: string; listType?: string; bold?: boolean; italic?: boolean };
  const stripTags = (s: string) => s.replace(/<[^>]+>/g, "");
  const decodeEntities = (s: string) => s
    .replace(/&nbsp;/g, " ").replace(/&amp;/g, "&").replace(/&lt;/g, "<")
    .replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&#39;/g, "'");
  function parseInline(inner: string): Node[] {
    const nodes: Node[] = [];
    const re = /<(\/?)(strong|b|em|i|a|br)(\s[^>]*)?>/gi;
    let last = 0;
    const stack: { bold: boolean; italic: boolean; url?: string }[] = [{ bold: false, italic: false }];
    const pushText = (raw: string) => {
      if (!raw) return;
      const top = stack[stack.length - 1];
      const text = decodeEntities(raw);
      if (!text) return;
      const node: Node = { type: "text", value: text };
      if (top.bold) node.bold = true;
      if (top.italic) node.italic = true;
      if (top.url) nodes.push({ type: "link", url: top.url, children: [node] });
      else nodes.push(node);
    };
    let m: RegExpExecArray | null;
    while ((m = re.exec(inner)) !== null) {
      pushText(inner.slice(last, m.index));
      const closing = m[1] === "/";
      const tag = m[2].toLowerCase();
      const attrs = m[3] ?? "";
      if (tag === "br") { /* ignore */ }
      else if (closing) { if (stack.length > 1) stack.pop(); }
      else {
        const top = { ...stack[stack.length - 1] };
        if (tag === "strong" || tag === "b") top.bold = true;
        if (tag === "em" || tag === "i") top.italic = true;
        if (tag === "a") {
          const h = /href=["']([^"']+)["']/i.exec(attrs);
          if (h) top.url = h[1];
        }
        stack.push(top);
      }
      last = m.index + m[0].length;
    }
    pushText(inner.slice(last));
    return nodes.length ? nodes : [{ type: "text", value: decodeEntities(stripTags(inner)) }];
  }
  const children: Node[] = [];
  const blockRe = /<(h[1-6]|p|ul|ol)(\s[^>]*)?>([\s\S]*?)<\/\1>/gi;
  let m: RegExpExecArray | null;
  let matched = false;
  while ((m = blockRe.exec(clean)) !== null) {
    matched = true;
    const tag = m[1].toLowerCase();
    const inner = m[3];
    if (tag.startsWith("h")) children.push({ type: "heading", level: parseInt(tag.slice(1), 10), children: parseInline(inner) });
    else if (tag === "p") children.push({ type: "paragraph", children: parseInline(inner) });
    else {
      const items: Node[] = [];
      const liRe = /<li(\s[^>]*)?>([\s\S]*?)<\/li>/gi;
      let li: RegExpExecArray | null;
      while ((li = liRe.exec(inner)) !== null) items.push({ type: "list-item", children: parseInline(li[2]) });
      children.push({ type: "list", listType: tag === "ol" ? "ordered" : "unordered", children: items });
    }
  }
  if (!matched) children.push({ type: "paragraph", children: parseInline(clean) });
  return JSON.stringify({ type: "root", children });
}

async function requireUser(req: Request) {
  const authHeader = req.headers.get("authorization");
  if (!authHeader) return false;
  if (authHeader.includes(SUPABASE_SERVICE_ROLE_KEY)) return true;
  const anon = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
    global: { headers: { Authorization: authHeader } },
  });
  const { data: { user }, error } = await anon.auth.getUser();
  return !error && Boolean(user);
}

async function shopifyGraphql(shopDomain: string, accessToken: string, query: string, variables: Record<string, unknown>) {
  const res = await fetch(`https://${shopDomain}/admin/api/${API_VERSION}/graphql.json`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "X-Shopify-Access-Token": accessToken },
    body: JSON.stringify({ query, variables }),
  });
  const data = await res.json();
  if (!res.ok || data.errors) throw new Error(`Shopify ${res.status}: ${JSON.stringify(data.errors || data)}`);
  return data.data;
}

async function getCheapestPurchasePrice(admin: any, masterProductId: string): Promise<number | null> {
  const { data } = await admin
    .from("supplier_products")
    .select("purchase_price, in_stock, stock_quantity")
    .eq("master_product_id", masterProductId);
  const rows = (data ?? [])
    .map((sp: any) => ({
      purchase: sp.purchase_price == null ? null : Number(sp.purchase_price),
      inStock: sp.in_stock === true && (sp.stock_quantity == null || Number(sp.stock_quantity) > 0),
    }))
    .filter((sp: { purchase: number | null }) => sp.purchase != null && Number.isFinite(sp.purchase) && sp.purchase > 0);
  if (rows.length === 0) return null;
  const inStockRows = rows.filter((sp: { inStock: boolean }) => sp.inStock);
  const pool = inStockRows.length > 0 ? inStockRows : rows;
  return Math.min(...pool.map((sp: { purchase: number }) => sp.purchase));
}

function assertNotBelowPurchase(sellingPriceInclVat: number | null, purchasePriceExVat: number | null, label: string) {
  if (sellingPriceInclVat == null || purchasePriceExVat == null) return;
  const sellingExVat = sellingPriceInclVat / (1 + VAT_RATE);
  if (sellingExVat + 0.005 < purchasePriceExVat) {
    throw new Error(
      `[${label}] Blokeret: salgspris ${sellingPriceInclVat.toFixed(2)} kr inkl. moms er under indkøb ${purchasePriceExVat.toFixed(2)} kr ekskl. moms.`
    );
  }
}

Deno.serve(async (req) => {
  if (req.method === "OPTIONS") return new Response("ok", { headers: corsHeaders });
  try {
    if (!(await requireUser(req))) {
      return new Response(JSON.stringify({ error: "Unauthorized" }), {
        status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    const { master_product_ids, option_name } = await req.json();
    if (!Array.isArray(master_product_ids) || master_product_ids.length < 2) {
      return new Response(JSON.stringify({ error: "master_product_ids (min 2) required" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    const optName = String(option_name || "").trim();
    if (!optName) {
      return new Response(JSON.stringify({ error: "option_name required" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);
    const { data: masters, error: pErr } = await supabase
      .from("master_products")
      .select("id, title, ean, sku, brand, category, webshop_price, sale_price, stock_quantity, short_description, long_description, meta_title, meta_description, lifecycle_status, shopify_product_id, weight_kg, backorder_policy, image_url, attributes")
      .in("id", master_product_ids);
    if (pErr || !masters || masters.length !== master_product_ids.length) {
      return new Response(JSON.stringify({ error: "One or more masters not found" }), {
        status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    for (const m of masters) {
      if (m.shopify_product_id) {
        return new Response(JSON.stringify({ error: `Master ${m.id} har allerede shopify_product_id` }), {
          status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
    }

    // Preserve input order for variants
    const ordered = master_product_ids.map((id: string) => masters.find((m: any) => m.id === id)!).filter(Boolean);

    // Validate + collect option values
    const optionValues: string[] = [];
    for (const m of ordered) {
      const attrs = (m.attributes ?? {}) as Record<string, unknown>;
      const val = attrs?.[optName];
      if (val == null || String(val).trim() === "") {
        throw new Error(`Master ${m.id} mangler attributten "${optName}"`);
      }
      const s = String(val).trim();
      if (optionValues.includes(s)) throw new Error(`Duplikeret optionsværdi: ${s}`);
      optionValues.push(s);
    }

    const { data: conn } = await supabase
      .from("shopify_connection")
      .select("shop_domain, access_token")
      .order("is_active", { ascending: false })
      .order("installed_at", { ascending: false })
      .limit(1).maybeSingle();
    if (!conn) {
      return new Response(JSON.stringify({ error: "Shopify er ikke forbundet" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    // Draft creation: warn but don't block below-cost variants. Update path still enforces guard on activation.
    for (const m of ordered) {
      const regular = m.webshop_price != null ? Number(m.webshop_price) : null;
      const sale = m.sale_price != null ? Number(m.sale_price) : null;
      const onSale = regular != null && sale != null && sale > 0 && sale < regular;
      const sellingPrice = onSale ? sale : regular;
      const cheapest = await getCheapestPurchasePrice(supabase, m.id);
      const label = String(m.attributes?.[optName] ?? m.id);
      if (sellingPrice != null && cheapest != null && sellingPrice / (1 + VAT_RATE) + 0.005 < cheapest) {
        console.warn(`shopify-create-product-with-variants: [${label}] Advarsel – salgspris ${sellingPrice.toFixed(2)} under indkøb ${cheapest.toFixed(2)}. Oprettes som KLADDE.`);
      }
    }
    void assertNotBelowPurchase;

    const head = ordered[0];

    // Step 1: productCreate with productOptions
    const productMutation = `#graphql
      mutation CreateProduct($input: ProductInput!, $media: [CreateMediaInput!]) {
        productCreate(input: $input, media: $media) {
          product { id }
          userErrors { field message }
        }
      }`;
    // Strip any trailing " - <optionValue>" suffix from the parent title so variant name
     // isn't duplicated on the Shopify product title.
    const stripSuffix = (t: string) => {
      let out = String(t ?? "").trim();
      for (const v of optionValues) {
        const re = new RegExp(`\\s*[-–—]\\s*${v.replace(/[.*+?^${}()|[\\]\\\\]/g, "\\$&")}\\s*$`, "i");
        out = out.replace(re, "").trim();
      }
      return out;
    };
    const parentTitle = stripSuffix(head.title);
    const productInput: Record<string, unknown> = {
      title: parentTitle,
      status: "DRAFT",
      descriptionHtml: head.long_description ?? "",
      vendor: head.brand ?? undefined,
      productType: head.category ?? undefined,
      productOptions: [{ name: optName, values: optionValues.map((v) => ({ name: v })) }],
    };
    if (head.short_description) {
      productInput.metafields = [{
        namespace: "custom",
        key: "shortdescription",
        type: "rich_text_field",
        value: htmlToShopifyRichText(String(head.short_description)),
      }];
    }
    const seoObj: Record<string, unknown> = {};
    if (head.meta_title) seoObj.title = String(head.meta_title);
    if (head.meta_description) seoObj.description = String(head.meta_description);
    if (Object.keys(seoObj).length > 0) productInput.seo = seoObj;

    const headImg = typeof head.image_url === "string" && /^https?:\/\//i.test(head.image_url) ? head.image_url : null;
    const created = await shopifyGraphql(conn.shop_domain, conn.access_token, productMutation, {
      input: productInput,
      media: headImg ? [{ originalSource: headImg, mediaContentType: "IMAGE", alt: parentTitle.slice(0, 250) }] : undefined,
    });
    const errs = created.productCreate.userErrors;
    if (errs?.length) throw new Error(errs.map((e: { message: string }) => e.message).join(", "));
    const productGid = created.productCreate.product.id as string;

    // Step 2: bulk create variants (removes standalone default variant)
    const variantsMutation = `#graphql
      mutation BulkCreate($productId: ID!, $variants: [ProductVariantsBulkInput!]!, $strategy: ProductVariantsBulkCreateStrategy) {
        productVariantsBulkCreate(productId: $productId, variants: $variants, strategy: $strategy) {
          productVariants { id barcode selectedOptions { name value } }
          userErrors { field message }
        }
      }`;
    const variantsInput = ordered.map((m: any) => {
      const regular = m.webshop_price != null ? Number(m.webshop_price) : null;
      const sale = m.sale_price != null ? Number(m.sale_price) : null;
      const onSale = regular != null && sale != null && sale > 0 && sale < regular;
      const sellingPrice = onSale ? sale : regular;
      const effectiveWeight = m.weight_kg != null ? Number(m.weight_kg) : 1;
      const inventoryPolicy = m.backorder_policy === "yes" ? "CONTINUE" : "DENY";
      return {
        optionValues: [{ optionName: optName, name: String(m.attributes?.[optName] ?? "") }],
        price: sellingPrice != null ? String(sellingPrice) : undefined,
        compareAtPrice: onSale ? String(regular) : undefined,
        barcode: m.ean ?? undefined,
        inventoryPolicy,
        inventoryItem: {
          sku: m.sku ?? m.ean ?? undefined,
          tracked: true,
          measurement: { weight: { value: effectiveWeight, unit: "KILOGRAMS" } },
        },
      };
    });

    const vData = await shopifyGraphql(conn.shop_domain, conn.access_token, variantsMutation, {
      productId: productGid,
      variants: variantsInput,
      strategy: "REMOVE_STANDALONE_VARIANT",
    });
    const vErrs = vData.productVariantsBulkCreate.userErrors;
    if (vErrs?.length) throw new Error(vErrs.map((e: { message: string }) => e.message).join(", "));

    const createdVariants = vData.productVariantsBulkCreate.productVariants as Array<{
      id: string; barcode: string | null; selectedOptions: Array<{ name: string; value: string }>;
    }>;

    const numericProductId = productGid.split("/").pop()!;
    const results: Array<{ master_id: string; shopify_variant_id: string }> = [];

    // Step 3: link each master to its variant
    for (const m of ordered) {
      const targetVal = String(m.attributes?.[optName] ?? "");
      const match = createdVariants.find((v) =>
        v.selectedOptions.some((o) => o.name === optName && o.value === targetVal)
      );
      if (!match) {
        console.error("No matching variant returned for", targetVal);
        continue;
      }
      const numericVariantId = match.id.split("/").pop()!;
      await supabase.from("master_products").update({
        shopify_product_id: numericProductId,
        shopify_variant_id: numericVariantId,
        shopify_sync_enabled: true,
        lifecycle_status: "pending_activation",
        updated_at: new Date().toISOString(),
      }).eq("id", m.id);

      await supabase.from("product_change_log").insert({
        master_product_id: m.id, change_type: "lifecycle", field_name: "lifecycle_status",
        old_value: m.lifecycle_status, new_value: "pending_activation", source: "shopify-create-product-with-variants",
      });
      results.push({ master_id: m.id, shopify_variant_id: numericVariantId });
    }

    return new Response(JSON.stringify({
      success: true,
      shopify_product_id: numericProductId,
      shopify_admin_url: `https://${conn.shop_domain}/admin/products/${numericProductId}`,
      variants: results,
      message: "Produkt oprettet i Shopify som KLADDE med varianter. Aktivér i Shopify-admin når klar.",
    }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    console.error("shopify-create-product-with-variants:", msg);
    return new Response(JSON.stringify({ error: msg }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  }
});
