// Adds a product to a Shopify custom collection and mirrors the link in PIM.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: col } = await supabase
      ...
      .select("id, shopify_collection_id, collection_type, title")
      .eq("id", collection_id)
      .single();
    ...
    await supabase
      ...
      .upsert({ master_product_id, collection_id }, { onConflict: "master_product_id,collection_id" });

    return new Response(JSON.stringify({ success: true }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error("shopify-collection-add-product:", msg);
