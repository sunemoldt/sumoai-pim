...
BEGIN
  ...
  IF v_product.auto_stock_sync
    ...
    -- Fallback: no supplier passes the margin filter → sum all in-stock
    -- suppliers at the cheapest available price tier, so we never lose real
    -- stock (fixes cases where a tie between suppliers dropped units).
    IF v_eligible_count = 0 OR v_total_stock = 0 THEN
      ...
      IF v_min_price IS NOT NULL THEN
        ...
        WHERE sp.master_product_id = p_master_product_id
          ...
          AND sp.in_stock = true
          AND (sp.stock_quantity IS NULL OR sp.stock_quantity > 0)
          AND sp.purchase_price = v_min_price;
        v_any_in_stock := v_total_stock > 0;
      ELSE
        v_total_stock := 0;
...
-- Recompute for the product the user reported so the fix is visible immediately.
SELECT public.recompute_product_stock(id)
FROM public.master_products
WHERE title ILIKE 'Ajax KeyPad TouchScreen%hvid%';
