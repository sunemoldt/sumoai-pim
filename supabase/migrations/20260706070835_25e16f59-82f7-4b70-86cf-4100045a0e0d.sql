...
-- Extend recompute_product_stock to read min_sync_margin_default from analytics_settings
...
-- Batch re-apply low margin guard across all products
CREATE OR REPLACE FUNCTION public.reapply_low_margin_guard_all()
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  LOOP
    PERFORM public.apply_low_margin_guard(r.id);
    cnt := cnt + 1;
  END LOOP;
  RETURN cnt;
END;
...
VALUES ('min_sync_margin_default', '15')
